...
int ezpf_AStarLength(
    struct ezpf_AStarContext *ctx, ezpf_NodeID s, ezpf_NodeID c)
{
    int res = 0;
...
void ezpf_AStarReconstruct(
    ezpf_NodeID *out_result,
    ...
    ezpf_NodeID c)
{
    ...
    {
        void *r          = realloc(ctx->neighbors, n * sizeof(ezpf_NodeID));
        ctx->neighbors   = r;
        ctx->neighborCap = n;
    }
